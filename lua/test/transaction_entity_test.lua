...
describe("TransactionEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Transaction(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Transaction(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Transaction(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set BLUEFINSHIELDCONEXMGMT_TEST_TRANSACTION_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local transaction_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.transaction")))
    local transaction_ref01_data = nil
    if #transaction_ref01_data_raw > 0 then
      transaction_ref01_data = helpers.to_map(transaction_ref01_data_raw[1][2])
    end
...
function transaction_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/transaction/TransactionTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read transaction test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("BLUEFINSHIELDCONEXMGMT_TEST_TRANSACTION_ENTID")
