...
describe("PartnerDirect", function()
  it("should direct-list-partner", function()
    local setup = partner_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-partner", setup.live and "live" or "unit")
  ...
  it("should direct-load-partner", function()
    local setup = partner_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-partner", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function partner_direct_setup(mockres)
