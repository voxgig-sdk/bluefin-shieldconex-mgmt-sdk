...
describe("TemplateDirect", function()
  it("should direct-list-template", function()
    local setup = template_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-template", setup.live and "live" or "unit")
  ...
  it("should direct-load-template", function()
    local setup = template_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-template", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function template_direct_setup(mockres)
