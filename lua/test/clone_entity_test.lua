...
describe("CloneEntity", function()
  it("should create instance", function()
    ...
    local ent = testsdk:Clone(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = clone_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "clone." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set BLUEFINSHIELDCONEXMGMT_TEST_CLONE_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local clone_ref01_ent = client:Clone(nil)
    local clone_ref01_data = helpers.to_map(vs.getprop(
      vs.getpath(setup.data, "new.clone"), "clone_ref01"))
    clone_ref01_data["template_id"] = setup.idmap["template01"]

    local clone_ref01_data_result, err = clone_ref01_ent:create(clone_ref01_data, nil)
    assert.is_nil(err)
    clone_ref01_data = helpers.to_map(clone_ref01_data_result)
    assert.is_not_nil(clone_ref01_data)
    assert.is_not_nil(clone_ref01_data["id"])
...
function clone_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/clone/CloneTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read clone test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("BLUEFINSHIELDCONEXMGMT_TEST_CLONE_ENTID")
