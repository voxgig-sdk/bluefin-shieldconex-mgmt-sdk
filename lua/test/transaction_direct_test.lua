...
describe("TransactionDirect", function()
  it("should direct-list-transaction", function()
    local setup = transaction_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-transaction", setup.live and "live" or "unit")
  ...
  it("should direct-load-transaction", function()
    local setup = transaction_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-transaction", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function transaction_direct_setup(mockres)
