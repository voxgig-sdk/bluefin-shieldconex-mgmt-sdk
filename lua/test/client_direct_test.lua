...
describe("ClientDirect", function()
  it("should direct-list-client", function()
    local setup = client_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-client", setup.live and "live" or "unit")
  ...
  it("should direct-load-client", function()
    local setup = client_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-client", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function client_direct_setup(mockres)
