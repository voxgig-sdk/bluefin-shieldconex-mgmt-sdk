...
describe("TemplateEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set BLUEFINSHIELDCONEXMGMT_TEST_TEMPLATE_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    local template_ref01_data_dt0_load_result = helpers.to_map(template_ref01_data_dt0_loaded)
...
function template_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("BLUEFINSHIELDCONEXMGMT_TEST_TEMPLATE_ENTID")
