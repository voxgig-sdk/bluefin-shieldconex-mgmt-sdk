...
TransactionEntity.__index = TransactionEntity


function TransactionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TransactionEntity)
  self._name = "transaction"
  self._client = client
...
function TransactionEntity:get_name()
  return self._name
end


function TransactionEntity:make()
  local opts = {}
...
function TransactionEntity:data_set(args)
  if args ~= nil then
...
function TransactionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TransactionEntity:match_set(args)
  if args ~= nil then
...
function TransactionEntity:match_get()
...
function TransactionEntity:stream(action, args, callopts)
...
---@param reqmatch TransactionLoadMatch
---@param ctrl? table
---@return Transaction
---@return string? err
function TransactionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch TransactionListMatch
---@param ctrl? table
---@return Transaction[]
---@return string? err
function TransactionEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function TransactionEntity:_run_op(ctx, post_done)
  local utility = self._utility
