# bluefin-shieldconex-mgmt-mcp

[MCP](https://modelcontextprotocol.io) server exposing the BluefinShieldconexMgmt SDK as
two agent tools — `bluefin-shieldconex-mgmt_list` and `bluefin-shieldconex-mgmt_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bluefin-shieldconex-mgmt-mcp)
make build

# 2. Provide credentials via the environment
export BLUEFIN_SHIELDCONEX_MGMT_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user bluefin-shieldconex-mgmt \
  -- /absolute/path/to/bluefin-shieldconex-mgmt-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./bluefin-shieldconex-mgmt-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// bluefin-shieldconex-mgmt_list: first page of records
{ "entity": "client" }
{ "entity": "client", "query": { } }

// bluefin-shieldconex-mgmt_load: one record by id
{ "entity": "client", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bluefin-shieldconex-mgmt-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user bluefin-shieldconex-mgmt \
     -- "$PWD"/dist/*/bluefin-shieldconex-mgmt-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `bluefin-shieldconex-mgmt_list` and `bluefin-shieldconex-mgmt_load` tools now appear
   in new sessions. Ask the agent to *"list client using bluefin-shieldconex-mgmt"*
   and it calls `bluefin-shieldconex-mgmt_list` with `{"entity":"client"}`.
...
```sh
export BLUEFIN_SHIELDCONEX_MGMT_APIKEY=sk_live_xxx            # API key
export BLUEFIN_SHIELDCONEX_MGMT_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `bluefin-shieldconex-mgmt_list` tool
...
### Call the `bluefin-shieldconex-mgmt_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `bluefin-shieldconex-mgmt_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `bluefin-shieldconex-mgmt_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BLUEFIN_SHIELDCONEX_MGMT_APIKEY` | API key sent with every request. |
| `BLUEFIN_SHIELDCONEX_MGMT_BASE` | Optional override of the API base URL. |

### Entities

The 7 entities valid as the `entity` argument:

client | clone | partner | template | transaction | update_result | user

### Smoke test via HTTP (raw JSON-RPC)

```sh
./bluefin-shieldconex-mgmt-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"bluefin-shieldconex-mgmt_load","arguments":{"entity":"client","query":{"id":1}}}}'
```
