// User entity client (generated — mirrors the rust Entity fragment).
...
static const char* user_get_name(Entity* e);
...
Entity* user_entity_new(BluefinShieldconexMgmtSDK* client, voxgig_value* entopts) {
  ...
  user_entity* self = (user_entity*)calloc(1, sizeof(user_entity));
  self->base.vt = &user_VT;
  self->name = strdup("user");
  self->client = client;
...
static voxgig_value* user_run_op(user_entity* self, Context* ctx,
                                    user_postdone_fn post_done, PNError** err) {
...
voxgig_value* user_stream(Entity* e, const char* action, voxgig_value* args,
...
static Entity* user_make(Entity* e) {
  ...
  return user_entity_new(self->client, opts);
}
...
static voxgig_value* user_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(user_ent_ctx(self), "SetData");
  }
  feature_hook_util(user_ent_ctx(self), "GetData");
...
static voxgig_value* user_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(user_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(user_ent_ctx(self), "GetMatch");
  return voxgig_clone(self->mtch);
}


static void user_load_postdone(user_entity* self, Context* ctx) {
...
static voxgig_value* user_remove(Entity* e, voxgig_value* reqarg, voxgig_value* ctrl, PNError** err) {
  (void)e; (void)reqarg; (void)ctrl;
  *err = unsupported_op("remove", "user");
  return NULL;
}
