// Clone entity client (generated — mirrors the rust Entity fragment).
...
static const char* clone_get_name(Entity* e);
...
Entity* clone_entity_new(BluefinShieldconexMgmtSDK* client, voxgig_value* entopts) {
  ...
  clone_entity* self = (clone_entity*)calloc(1, sizeof(clone_entity));
  self->base.vt = &clone_VT;
  self->name = strdup("clone");
  self->client = client;
...
static voxgig_value* clone_run_op(clone_entity* self, Context* ctx,
                                    clone_postdone_fn post_done, PNError** err) {
...
voxgig_value* clone_stream(Entity* e, const char* action, voxgig_value* args,
...
static Entity* clone_make(Entity* e) {
  ...
  return clone_entity_new(self->client, opts);
}
...
static voxgig_value* clone_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(clone_ent_ctx(self), "SetData");
  }
  feature_hook_util(clone_ent_ctx(self), "GetData");
...
static voxgig_value* clone_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(clone_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(clone_ent_ctx(self), "GetMatch");
...
static void clone_create_postdone(clone_entity* self, Context* ctx) {
