// Client entity client (generated — mirrors the rust Entity fragment).
...
static const char* client_get_name(Entity* e);
...
Entity* client_entity_new(BluefinShieldconexMgmtSDK* client, voxgig_value* entopts) {
  ...
  client_entity* self = (client_entity*)calloc(1, sizeof(client_entity));
  self->base.vt = &client_VT;
  self->name = strdup("client");
  self->client = client;
...
static voxgig_value* client_run_op(client_entity* self, Context* ctx,
                                    client_postdone_fn post_done, PNError** err) {
...
voxgig_value* client_stream(Entity* e, const char* action, voxgig_value* args,
...
static Entity* client_make(Entity* e) {
  ...
  return client_entity_new(self->client, opts);
}
...
static voxgig_value* client_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(client_ent_ctx(self), "SetData");
  }
  feature_hook_util(client_ent_ctx(self), "GetData");
...
static voxgig_value* client_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(client_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(client_ent_ctx(self), "GetMatch");
  return voxgig_clone(self->mtch);
}


static void client_load_postdone(client_entity* self, Context* ctx) {
...
static void client_list_postdone(client_entity* self, Context* ctx) {
  SdkResult* result = ctx->result;
  if (result) {
    voxgig_value* resmatch = result->resmatch;
    if (voxgig_is_map(resmatch)) self->mtch = resmatch;
...
static void client_create_postdone(client_entity* self, Context* ctx) {
  SdkResult* result = ctx->result;
  if (result) {
    voxgig_value* resdata = result->resdata;
    if (!v_is_noval(resdata) && !v_is_null(resdata)) {
...
static void client_remove_postdone(client_entity* self, Context* ctx) {
  SdkResult* result = ctx->result;
  if (result) {
    voxgig_value* resmatch = result->resmatch;
    voxgig_value* resdata = result->resdata;
