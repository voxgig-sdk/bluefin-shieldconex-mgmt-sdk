// BluefinShieldconexMgmtSDK client (generated — mirrors the rust Main fragment).
...
BluefinShieldconexMgmtSDK* bluefinshieldconexmgmt_sdk_new(voxgig_value* options) {
  BluefinShieldconexMgmtSDK* sdk = (BluefinShieldconexMgmtSDK*)calloc(1, sizeof(BluefinShieldconexMgmtSDK));
...
BluefinShieldconexMgmtSDK* test_sdk(voxgig_value* testopts, voxgig_value* sdkopts) {
  ...
  BluefinShieldconexMgmtSDK* sdk = bluefinshieldconexmgmt_sdk_new(sdkopts);
  free(sdk->mode);
  sdk->mode = strdup("test");
  return sdk;
}
