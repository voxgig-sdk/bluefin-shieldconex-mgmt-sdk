// Generated direct-call test for the transaction entity (mirrors the
...
static voxgig_value* transaction_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinShieldconexMgmtSDK* transaction_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(transaction_mock, mockres)));
...
int main(void) {
  ...
  // LIST
  {
    ...
    BluefinShieldconexMgmtSDK* sdk = transaction_direct_setup(mockres);
    voxgig_value* params = v_map();
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("transactions"),
      "method", v_str("GET"),
      "params", params), &err);
  ...
  TEST_SUMMARY("bluefin_shieldconex_mgmt_transaction_direct");
}
