// Generated direct-call test for the partner entity (mirrors the
...
static voxgig_value* partner_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinShieldconexMgmtSDK* partner_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(partner_mock, mockres)));
...
int main(void) {
  ...
  // LIST
  {
    ...
    BluefinShieldconexMgmtSDK* sdk = partner_direct_setup(mockres);
  ...
  TEST_SUMMARY("bluefin_shieldconex_mgmt_partner_direct");
}
