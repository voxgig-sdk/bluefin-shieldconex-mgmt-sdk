// Generated direct-call test for the template entity (mirrors the
...
static voxgig_value* template_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinShieldconexMgmtSDK* template_direct_setup(voxgig_value* mockres) {
...
int main(void) {
  ...
  // LIST
  {
    ...
    BluefinShieldconexMgmtSDK* sdk = template_direct_setup(mockres);
    voxgig_value* params = v_map();
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("templates"),
      "method", v_str("GET"),
      "params", params), &err);
