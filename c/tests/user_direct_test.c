// Generated direct-call test for the user entity (mirrors the
...
static voxgig_value* user_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinShieldconexMgmtSDK* user_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(user_mock, mockres)));
...
int main(void) {
  ...
  // LOAD
  {
    ...
    BluefinShieldconexMgmtSDK* sdk = user_direct_setup(mockres);
  ...
  TEST_SUMMARY("bluefin_shieldconex_mgmt_user_direct");
}
