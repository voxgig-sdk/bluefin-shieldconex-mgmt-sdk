// Generated direct-call test for the client entity (mirrors the
...
static voxgig_value* client_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinShieldconexMgmtSDK* client_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(client_mock, mockres)));
...
int main(void) {
  ...
  // LIST
  {
    ...
    BluefinShieldconexMgmtSDK* sdk = client_direct_setup(mockres);
  ...
  TEST_SUMMARY("bluefin_shieldconex_mgmt_client_direct");
}
