# BluefinShieldconexMgmt SDK utility: make_options
...
$REGISTRY{make_options} = sub {
  ...
  my $custom_utils = BluefinShieldconexMgmtHelpers::gp($options, 'utility');
  ...
  my @featureorder;
  my $feature_raw = BluefinShieldconexMgmtHelpers::gp($opts, 'feature');
  if (Voxgig::Struct::islist($feature_raw)) {
    my %fmap;
    for my $entry (@$feature_raw) {
      next unless Voxgig::Struct::ismap($entry);
      my $name = BluefinShieldconexMgmtHelpers::gp($entry, 'name');
  ...
  BluefinShieldconexMgmtHelpers::coerce_bools($opts);
  ...
  my $sys_fetch = BluefinShieldconexMgmtHelpers::gpath($opts, 'system.fetch');
  ...
  my $clean_keys = BluefinShieldconexMgmtHelpers::gpath($opts, 'clean.keys');
  ...
  if (!@featureorder) {
    my $fmap = BluefinShieldconexMgmtHelpers::to_map(
      BluefinShieldconexMgmtHelpers::gp($opts, 'feature')) || {};
