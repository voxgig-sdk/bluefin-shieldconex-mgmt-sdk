...
package BluefinShieldconexMgmtContext;
...
sub new {
  ...
  my $gcp = \&BluefinShieldconexMgmtHelpers::get_ctx_prop;
  ...
  $self->{ctrl} = BluefinShieldconexMgmtControl->new();
  ...
  $self->{data} = BluefinShieldconexMgmtHelpers::to_map($gcp->($ctxmap, 'data')) || {};
  $self->{reqdata} = BluefinShieldconexMgmtHelpers::to_map($gcp->($ctxmap, 'reqdata')) || {};
  $self->{match} = BluefinShieldconexMgmtHelpers::to_map($gcp->($ctxmap, 'match')) || {};
  $self->{reqmatch} = BluefinShieldconexMgmtHelpers::to_map($gcp->($ctxmap, 'reqmatch')) || {};

  my $pt = $gcp->($ctxmap, 'point');
  $self->{point} = Voxgig::Struct::ismap($pt) ? $pt : ($basectx ? $basectx->{point} : undef);

  my $sp = $gcp->($ctxmap, 'spec');
  $self->{spec} = (Scalar::Util::blessed($sp) && $sp->isa('BluefinShieldconexMgmtSpec'))
    ? $sp : ($basectx ? $basectx->{spec} : undef);

  my $r = $gcp->($ctxmap, 'result');
  $self->{result} = (Scalar::Util::blessed($r) && $r->isa('BluefinShieldconexMgmtResult'))
    ? $r : ($basectx ? $basectx->{result} : undef);

  my $rp = $gcp->($ctxmap, 'response');
  $self->{response} = (Scalar::Util::blessed($rp) && $rp->isa('BluefinShieldconexMgmtResponse'))
...
sub resolve_op {
  ...
  return BluefinShieldconexMgmtOperation->new({}) if '' eq $opname;

  my $opcfg = BluefinShieldconexMgmtHelpers::gpath($self->{config}, "entity.$entname.op.$opname");

  my $input = ($opname eq 'update' || $opname eq 'create') ? 'data' : 'match';

  my $points = [];
  if (Voxgig::Struct::ismap($opcfg)) {
    my $t = BluefinShieldconexMgmtHelpers::gp($opcfg, 'points');
    $points = $t if Voxgig::Struct::islist($t);
  }

  my $op = BluefinShieldconexMgmtOperation->new({
    entity => $entname,
    name   => $opname,
    input  => $input,
    points => $points,
...
sub make_error {
  my ($self, $code, $msg) = @_;
  return BluefinShieldconexMgmtError->new($code, $msg, $self);
}
