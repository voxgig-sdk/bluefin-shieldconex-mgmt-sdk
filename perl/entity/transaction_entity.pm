...
sub make {
  ...
  return TransactionEntity->new($self->{_client}, $opts);
}
...
# Returns the current Transaction data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Transaction fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; TransactionLoadMatch
...
# Returns the loaded Transaction data (hashref); dies with BluefinShieldconexMgmtError
# on failure.
sub load {
...
#
# reqmatch: match filter hashref (any subset of Transaction fields;
# TransactionListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching Transaction items as an arrayref; dies with
