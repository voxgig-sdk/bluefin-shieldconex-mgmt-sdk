# BluefinShieldconexMgmt SDK Template entity
...
sub new {
  ...
  elsif (BluefinShieldconexMgmtHelpers::is_false($entopts->{active})) {
    # keep false
  }
  else {
    $entopts->{active} = Voxgig::Struct::JTRUE();
...
sub make {
  ...
  return TemplateEntity->new($self->{_client}, $opts);
}

sub data_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_data} = BluefinShieldconexMgmtHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetData');
  }
  return;
}

# Returns the current Template data (hashref).
sub data_get {
  my ($self) = @_;
...
sub match_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_match} = BluefinShieldconexMgmtHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetMatch');
  }
  return;
}

# Returns the current match filter (any subset of Template fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; TemplateLoadMatch
...
# Returns the loaded Template data (hashref); dies with BluefinShieldconexMgmtError
# on failure.
sub load {
  ...
  return $self->_run_op($ctx, sub {
    ...
    if ($result) {
      ...
      if ($result->{resdata}) {
        $self->{_data} = BluefinShieldconexMgmtHelpers::to_map(
...
# List Template items matching the given filter.
#
# reqmatch: match filter hashref (any subset of Template fields;
# TemplateListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching Template items as an arrayref; dies with
# BluefinShieldconexMgmtError on failure.
sub list {
  my ($self, $reqmatch, $ctrl) = @_;
  my $utility = $self->{_utility};
  my $ctx = $utility->{make_context}->({
...
#
# reqdata: body data hashref (TemplateCreateData shape). ctrl: optional
# per-call control. Returns the created Template data (hashref); dies
# with BluefinShieldconexMgmtError on failure.
sub create {
...
# Remove an Template matching the given criteria.
#
# reqmatch: match criteria hashref (id/query fields; TemplateRemoveMatch
# shape). ctrl: optional per-call control. Returns the removed Template
# data (hashref); dies with BluefinShieldconexMgmtError on failure.
sub remove {
  my ($self, $reqmatch, $ctrl) = @_;
  my $utility = $self->{_utility};
  my $ctx = $utility->{make_context}->({
...
sub stream {
  ...
  my $utility = $self->{_utility};
  $callopts = BluefinShieldconexMgmtHelpers::to_map($callopts) || {};
  my $signal = BluefinShieldconexMgmtHelpers::gp($callopts, 'signal');
  my $ctrl = BluefinShieldconexMgmtHelpers::to_map(
    BluefinShieldconexMgmtHelpers::gp($callopts, 'ctrl')) || {};
  ...
  my $ctx = $utility->{make_context}->({
    ...
    'match' => $self->{_match},
    'data' => $self->{_data},
    %{ BluefinShieldconexMgmtHelpers::to_map($args) || {} },
  }, $self->{_entctx});

  # Outbound: expose an async-iterable/list payload so the request builder /
  # transport can stream it as the request body.
  my $body = BluefinShieldconexMgmtHelpers::gp($callopts, 'body');
  if (defined $body) {
    my $reqdata = BluefinShieldconexMgmtHelpers::to_map($ctx->{reqdata}) || {};
    $reqdata->{'body$'} = $body;
    $ctx->{reqdata} = $reqdata;
    $ctx->{stream_out} = $body;
  }
