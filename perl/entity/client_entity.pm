# BluefinShieldconexMgmt SDK Client entity
...
sub make {
  ...
  return ClientEntity->new($self->{_client}, $opts);
}
...
# Returns the current Client data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Client fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; ClientLoadMatch
...
# Returns the loaded Client data (hashref); dies with BluefinShieldconexMgmtError
# on failure.
sub load {
...
# List Client items matching the given filter.
#
# reqmatch: match filter hashref (any subset of Client fields;
# ClientListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching Client items as an arrayref; dies with
...
# Create a new Client.
#
# reqdata: body data hashref (ClientCreateData shape). ctrl: optional
# per-call control. Returns the created Client data (hashref); dies
...
# Remove an Client matching the given criteria.
#
# reqmatch: match criteria hashref (id/query fields; ClientRemoveMatch
# shape). ctrl: optional per-call control. Returns the removed Client
