# BluefinShieldconexMgmt SDK Partner entity
...
sub make {
  ...
  return PartnerEntity->new($self->{_client}, $opts);
}
...
# Returns the current Partner data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Partner fields).
sub match_get {
  my ($self) = @_;
...
# Load a single Partner.
#
# reqmatch: match criteria hashref (id/query fields; PartnerLoadMatch
...
# Returns the loaded Partner data (hashref); dies with BluefinShieldconexMgmtError
# on failure.
sub load {
...
# List Partner items matching the given filter.
#
# reqmatch: match filter hashref (any subset of Partner fields;
# PartnerListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching Partner items as an arrayref; dies with
...
#
# reqdata: body data hashref (PartnerCreateData shape). ctrl: optional
# per-call control. Returns the created Partner data (hashref); dies
