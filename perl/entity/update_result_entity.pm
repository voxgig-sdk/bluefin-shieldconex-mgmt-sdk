...
sub make {
  ...
  return UpdateResultEntity->new($self->{_client}, $opts);
}
...
# Returns the current UpdateResult data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of UpdateResult fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match filter hashref (any subset of UpdateResult fields;
# UpdateResultListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching UpdateResult items as an arrayref; dies with
...
#
# reqdata: body data hashref (UpdateResultCreateData shape). ctrl: optional
# per-call control. Returns the created UpdateResult data (hashref); dies
...
#
# reqdata: body data hashref (UpdateResultUpdateData shape). ctrl: optional
# per-call control. Returns the updated UpdateResult data (hashref); dies
# with BluefinShieldconexMgmtError on failure.
sub update {
  my ($self, $reqdata, $ctrl) = @_;
  my $utility = $self->{_utility};
