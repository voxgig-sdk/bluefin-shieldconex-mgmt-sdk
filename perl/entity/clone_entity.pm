# BluefinShieldconexMgmt SDK Clone entity
...
sub make {
  ...
  return CloneEntity->new($self->{_client}, $opts);
}
...
# Returns the current Clone data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Clone fields).
sub match_get {
  my ($self) = @_;
...
# Create a new Clone.
#
# reqdata: body data hashref (CloneCreateData shape). ctrl: optional
# per-call control. Returns the created Clone data (hashref); dies
