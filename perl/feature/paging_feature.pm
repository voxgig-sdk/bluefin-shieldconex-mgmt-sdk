# BluefinShieldconexMgmt SDK paging feature
#
...
package BluefinShieldconexMgmtPagingFeature;
...
sub init {
  ...
  $self->{active} = BluefinShieldconexMgmtHelpers::is_true($self->{options}{active});
  return;
}
...
sub PreResult {
  ...
  if (Voxgig::Struct::ismap($body)) {
    my $bnext = BluefinShieldconexMgmtHelpers::gp($body, 'next');
    $paging->{next} = defined $paging->{next} ? $paging->{next} : $bnext
      if defined $bnext;
    my $bcursor = BluefinShieldconexMgmtHelpers::gp($body, 'cursor');
    $paging->{cursor} = $bcursor if defined $bcursor;
    my $bnextcursor = BluefinShieldconexMgmtHelpers::gp($body, 'nextCursor');
    $paging->{cursor} = $bnextcursor if defined $bnextcursor;
    my $bhasmore = $body->{hasMore};
    if (BluefinShieldconexMgmtHelpers::is_true($bhasmore) || BluefinShieldconexMgmtHelpers::is_false($bhasmore)) {
      $paging->{hasMore} = BluefinShieldconexMgmtHelpers::is_true($bhasmore) ? 1 : 0;
    }
  }
