...
package BluefinShieldconexMgmtTestFeature;
...
sub init {
  ...
  my $entity = BluefinShieldconexMgmtHelpers::gp($options, 'entity');
  ...
  my $test_fetcher = sub {
    ...
    my $op = $fctx->{op};
    my $entmap = BluefinShieldconexMgmtHelpers::gp($entity, $op->{entity});
    ...
    my $resolve_match = sub {
      ...
      for my $src ($fctx->{match}, $fctx->{data}) {
        ...
        my $v = BluefinShieldconexMgmtHelpers::gp($src, 'id');
    ...
    if ('load' eq $op->{name}) {
      ...
      my $ent = BluefinShieldconexMgmtHelpers::ge($found, 0);
      return $respond->(404, undef, { 'statusText' => 'Not found' })
        unless BluefinShieldconexMgmtHelpers::rb_truthy($ent);
      ...
      if (Voxgig::Struct::ismap($fctx->{reqdata})) {
        ...
        if ($op->{alias}) {
          my $alias_id = BluefinShieldconexMgmtHelpers::gp($op->{alias}, 'id');
      ...
      if (Voxgig::Struct::ismap($ent)) {
        my $id = BluefinShieldconexMgmtHelpers::gp($ent, 'id');
  ...
  my $net = BluefinShieldconexMgmtHelpers::gp($options, 'net');
...
sub make_netsim {
  ...
  my $do_sleep = sub {
    ...
    if (ref $net->{sleep} eq 'CODE') {
      ...
      BluefinShieldconexMgmtHelpers::sleep_ms($ms);
    }
  };
  ...
  return sub {
    ...
    if (BluefinShieldconexMgmtHelpers::is_true($net->{offline})) {
...
sub build_args {
  ...
  my $points = BluefinShieldconexMgmtHelpers::gpath($ctx->{config},
    "entity.$entname.op.$opname.points");
  my $point = BluefinShieldconexMgmtHelpers::ge($points, -1);

  my $params_path = BluefinShieldconexMgmtHelpers::gpath($point, 'args.params');
    ...
    if ($keys) {
      for my $key (@$keys) {
        ...
        if ($is_id || $is_reqd) {
          ...
          my $ka = $op->{alias} ? BluefinShieldconexMgmtHelpers::gp($op->{alias}, $key) : undef;
