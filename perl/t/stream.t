...
sub has_feature {
  my ($name) = @_;
  my $f = BluefinShieldconexMgmtConfig::make_config()->{feature};
...
sub find_list_entity {
  ...
  my $entities = BluefinShieldconexMgmtHelpers::to_map(
    BluefinShieldconexMgmtHelpers::gp($config, 'entity')) || {};
  for my $name (sort keys %$entities) {
    my $points = BluefinShieldconexMgmtHelpers::gpath($config, "entity.$name.op.list.points");
    next unless Voxgig::Struct::islist($points);
    for my $p (@$points) {
      my $params = BluefinShieldconexMgmtHelpers::gpath($p, 'args.params');
      my $reqd = Voxgig::Struct::islist($params)
        ? scalar(grep { BluefinShieldconexMgmtHelpers::is_true($_->{reqd}) } @$params)
        : 0;
...
SKIP: {
  ...
  {
    my $sdk = BluefinShieldconexMgmtSDK->test({ 'entity' => $seed }, undef);
  ...
  if (has_feature('streaming')) {
    {
      my $sdk = BluefinShieldconexMgmtSDK->test(
        { 'entity' => $seed },
        { 'feature' => { 'streaming' => {
