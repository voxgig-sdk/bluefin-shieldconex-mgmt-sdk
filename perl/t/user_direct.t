...
DIRECT_LOAD: {
  my $setup = user_direct_setup({ 'id' => 'direct01' });
  my ($_should_skip, $_reason) = BluefinShieldconexMgmtTestRunner::is_control_skipped(
    'direct', 'direct-load-user', $setup->{live} ? 'live' : 'unit');
  if ($_should_skip) {
    note($_reason || 'skipped via sdk-test-control.json');
    pass('direct-load-user: skipped via sdk-test-control.json');
    last DIRECT_LOAD;
  }
  if ($setup->{live}) {
    note('live direct-load needs real ID - set *_ENTID env var with real IDs to run');
    pass('direct-load-user: skipped (live)');
    last DIRECT_LOAD;
  }
  ...
  if ($setup->{live}) {
    ...
    pass('direct-load-user: live ok');
  }
  else {
    ok(!defined $result->{err}, 'direct-load-user: no error');
    ok($result->{ok}, 'direct-load-user: ok');
    is(BluefinShieldconexMgmtHelpers::to_int($result->{status}), 200, 'direct-load-user: status');
    ok(defined $result->{data}, 'direct-load-user: data');
    if (Voxgig::Struct::ismap($result->{data})) {
      is($result->{data}{id}, 'direct01', 'direct-load-user: id');
    }
    is(scalar @{ $setup->{calls} }, 1, 'direct-load-user: 1 call');
  }
}


sub user_direct_setup {
  my ($mockres) = @_;
