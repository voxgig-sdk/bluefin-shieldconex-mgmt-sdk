...
package BluefinShieldconexMgmtTestRunner;
...
sub is_control_skipped {
  ...
  my $skip = BluefinShieldconexMgmtHelpers::gpath($ctrl, "test.skip.$mode") || {};
  my $items = $skip->{$kind} || [];
...
sub live_delay_ms {
  my $ctrl = load_test_control();
  my $v = BluefinShieldconexMgmtHelpers::gpath($ctrl, 'test.live.delayMs');
