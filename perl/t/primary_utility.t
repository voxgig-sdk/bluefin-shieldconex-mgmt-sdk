...

{
  ...
  our @ISA = ('BluefinShieldconexMgmtBaseFeature');

  sub new {
    my ($class, $fn) = @_;
    my $self = BluefinShieldconexMgmtBaseFeature::new($class);
    $self->{hook_fn} = $fn;
    return $self;
  }
...
sub make_ctx_from_map {
  ...
  my $ctx = BluefinShieldconexMgmtContext->new($ctxmap, undef);
  ...
  if (Voxgig::Struct::ismap($ctxmap->{response})) {
    ...
    $ctx->{response} = BluefinShieldconexMgmtResponse->new($resp_map);
    if (BluefinShieldconexMgmtHelpers::rb_truthy($resp_map->{body})) {
...
sub err_from_map {
  ...
  return BluefinShieldconexMgmtError->new($code, $msg);
}
...

{
  ...
  my $feature = BluefinShieldconexMgmtBaseFeature->new;
...
{
  my $hook_client = BluefinShieldconexMgmtSDK->test(undef, undef);
...
{
  my $init_client = BluefinShieldconexMgmtSDK->test(undef, undef);
...
{
  my $calls = [];
  my $live_client = BluefinShieldconexMgmtSDK->new({
    'system' => {
      'fetch' => sub {
        my ($url, $fetchdef) = @_;
...
{
  my $blocked_client = BluefinShieldconexMgmtSDK->new({
    'system' => {
...
{
  my $ctx = make_test_full_ctx($client, $utility);
  $ctx->{spec} = BluefinShieldconexMgmtSpec->new({ 'step' => 'start' });
  $ctx->{result} = undef;
  my (undef, $err) = $utility->{make_result}->($ctx);
...

{
  ...
  my $spec_client = BluefinShieldconexMgmtSDK->test(undef, $setup_opts);
...
runset('makeUrl.basic', get_spec($PRIMARY, 'makeUrl', 'basic'), sub {
  ...
  my $ctxmap = $entry->{ctx} || {};
  my $ctx = make_ctx_from_map($ctxmap, $client, $utility);
  $ctx->{result} = BluefinShieldconexMgmtResult->new({}) unless $ctx->{result};
...
runset('operator.basic', get_spec($PRIMARY, 'operator', 'basic'), sub {
  my ($entry) = @_;
  my $in_val = $entry->{in} || {};
  my $op = BluefinShieldconexMgmtOperation->new($in_val);
  return {
    'entity' => $op->{entity},
    'name' => $op->{name},
    'input' => $op->{input},
...

{
  ...
  my $auth_client = BluefinShieldconexMgmtSDK->test(undef, $setup_opts);
