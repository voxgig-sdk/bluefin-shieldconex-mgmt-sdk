// Client entity client (generated — mirrors the go/rust Entity fragment).
...
pub const ClientEntity = struct {
    ...
    pub fn new(client: *sdk.BluefinShieldconexMgmtSDK, entopts_in: Value) *ClientEntity {
        ...
        const e = h.A().create(ClientEntity) catch unreachable;
        e.* = .{
            .name = "client",
            .client = client,
    ...
    fn ent_ctx(self: *ClientEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *ClientEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *ClientEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *ClientEntity, ctx: *Context, post_done: *const fn (*ClientEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *ClientEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *ClientEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return ClientEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *ClientEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *ClientEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *ClientEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *ClientEntity, reqmatch: Value, ctrl: Value) OpResult {
    ...
    fn load_post_done(self: *ClientEntity, ctx: *Context) void {
    ...
    pub fn list(self: *ClientEntity, reqmatch: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn list_post_done(self: *ClientEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            if (resmatch == .object) self.mtch = resmatch;
        }
    ...
    pub fn create(self: *ClientEntity, reqdata: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *ClientEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resdata = result.resdata;
            if (!h.is_noval(resdata)) {
                const cm = h.to_map(h.clone(resdata));
    ...
    pub fn update(self: *ClientEntity, _reqdata: Value, _ctrl: Value) OpResult {
        _ = _reqdata;
        _ = _ctrl;
    ...
    pub fn remove(self: *ClientEntity, reqmatch: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "remove",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn remove_post_done(self: *ClientEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            const resdata = result.resdata;
            if (resmatch == .object) self.mtch = resmatch;
