// User entity client (generated — mirrors the go/rust Entity fragment).
...
pub const UserEntity = struct {
    ...
    pub fn new(client: *sdk.BluefinShieldconexMgmtSDK, entopts_in: Value) *UserEntity {
        ...
        const e = h.A().create(UserEntity) catch unreachable;
        e.* = .{
            .name = "user",
            .client = client,
    ...
    fn ent_ctx(self: *UserEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *UserEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *UserEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *UserEntity, ctx: *Context, post_done: *const fn (*UserEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *UserEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *UserEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return UserEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *UserEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *UserEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *UserEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *UserEntity, reqmatch: Value, ctrl: Value) OpResult {
    ...
    fn load_post_done(self: *UserEntity, ctx: *Context) void {
    ...
    pub fn list(self: *UserEntity, _reqmatch: Value, _ctrl: Value) OpResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("list", self.name) };
    }

    pub fn create(self: *UserEntity, _reqdata: Value, _ctrl: Value) OpResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *UserEntity, _reqdata: Value, _ctrl: Value) OpResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *UserEntity, _reqmatch: Value, _ctrl: Value) OpResult {
        _ = _reqmatch;
        _ = _ctrl;
