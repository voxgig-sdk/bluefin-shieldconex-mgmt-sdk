// Transaction entity client (generated — mirrors the go/rust Entity fragment).
...
pub const TransactionEntity = struct {
    name: []const u8 = "transaction",
    ...
    pub fn new(client: *sdk.BluefinShieldconexMgmtSDK, entopts_in: Value) *TransactionEntity {
        ...
        const e = h.A().create(TransactionEntity) catch unreachable;
        e.* = .{
            .name = "transaction",
            .client = client,
    ...
    fn ent_ctx(self: *TransactionEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *TransactionEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *TransactionEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *TransactionEntity, ctx: *Context, post_done: *const fn (*TransactionEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *TransactionEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *TransactionEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return TransactionEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *TransactionEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *TransactionEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *TransactionEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *TransactionEntity, reqmatch: Value, ctrl: Value) OpResult {
    ...
    fn load_post_done(self: *TransactionEntity, ctx: *Context) void {
    ...
    pub fn list(self: *TransactionEntity, reqmatch: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn list_post_done(self: *TransactionEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            if (resmatch == .object) self.mtch = resmatch;
        }
    ...
    pub fn create(self: *TransactionEntity, _reqdata: Value, _ctrl: Value) OpResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *TransactionEntity, _reqdata: Value, _ctrl: Value) OpResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *TransactionEntity, _reqmatch: Value, _ctrl: Value) OpResult {
        _ = _reqmatch;
        _ = _ctrl;
