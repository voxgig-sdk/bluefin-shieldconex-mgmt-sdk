// UpdateResult entity client (generated — mirrors the go/rust Entity fragment).
...
pub const UpdateResultEntity = struct {
    name: []const u8 = "update_result",
    client: *sdk.BluefinShieldconexMgmtSDK,
    utility: *Utility,
    entopts: Value,
    data: Value,
    mtch: Value,
    ...
    pub fn new(client: *sdk.BluefinShieldconexMgmtSDK, entopts_in: Value) *UpdateResultEntity {
        ...
        const e = h.A().create(UpdateResultEntity) catch unreachable;
        e.* = .{
            .name = "update_result",
            .client = client,
    ...
    fn ent_ctx(self: *UpdateResultEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *UpdateResultEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *UpdateResultEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *UpdateResultEntity, ctx: *Context, post_done: *const fn (*UpdateResultEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *UpdateResultEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *UpdateResultEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return UpdateResultEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *UpdateResultEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *UpdateResultEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *UpdateResultEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *UpdateResultEntity, _reqmatch: Value, _ctrl: Value) OpResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn list(self: *UpdateResultEntity, reqmatch: Value, ctrl: Value) OpResult {
    ...
    fn list_post_done(self: *UpdateResultEntity, ctx: *Context) void {
    ...
    pub fn create(self: *UpdateResultEntity, reqdata: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *UpdateResultEntity, ctx: *Context) void {
        if (ctx.result) |result| {
    ...
    pub fn update(self: *UpdateResultEntity, reqdata: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "update",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn update_post_done(self: *UpdateResultEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            const resdata = result.resdata;
            if (resmatch == .object) self.mtch = resmatch;
    ...
    pub fn remove(self: *UpdateResultEntity, _reqmatch: Value, _ctrl: Value) OpResult {
        _ = _reqmatch;
        _ = _ctrl;
