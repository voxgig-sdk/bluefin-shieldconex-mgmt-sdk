// Partner entity client (generated — mirrors the go/rust Entity fragment).
...
pub const PartnerEntity = struct {
    name: []const u8 = "partner",
    ...
    pub fn new(client: *sdk.BluefinShieldconexMgmtSDK, entopts_in: Value) *PartnerEntity {
        ...
        const e = h.A().create(PartnerEntity) catch unreachable;
        e.* = .{
            .name = "partner",
            .client = client,
    ...
    fn ent_ctx(self: *PartnerEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *PartnerEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *PartnerEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *PartnerEntity, ctx: *Context, post_done: *const fn (*PartnerEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *PartnerEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *PartnerEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return PartnerEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *PartnerEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *PartnerEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *PartnerEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *PartnerEntity, reqmatch: Value, ctrl: Value) OpResult {
    ...
    fn load_post_done(self: *PartnerEntity, ctx: *Context) void {
    ...
    pub fn list(self: *PartnerEntity, reqmatch: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn list_post_done(self: *PartnerEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            if (resmatch == .object) self.mtch = resmatch;
        }
    ...
    pub fn create(self: *PartnerEntity, reqdata: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *PartnerEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resdata = result.resdata;
            if (!h.is_noval(resdata)) {
                const cm = h.to_map(h.clone(resdata));
    ...
    pub fn update(self: *PartnerEntity, _reqdata: Value, _ctrl: Value) OpResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *PartnerEntity, _reqmatch: Value, _ctrl: Value) OpResult {
        _ = _reqmatch;
        _ = _ctrl;
