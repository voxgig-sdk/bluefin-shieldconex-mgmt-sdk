// update_result entity test (generated from the API model).

import XCTest

@testable import BluefinShieldconexMgmtSdk

final class UpdateResultEntityTest: XCTestCase {
  func testInstance() {
    let sdk = BluefinShieldconexMgmtSDK.testSDK(nil, nil)
    let ent = sdk.UpdateResult()
    XCTAssertEqual(ent.getName(), "update_result")
  }

  func testStream() async throws {
    // Seed two records (under the test feature's entity key) and activate
    // the streaming feature.
    let fixtures = vm(("entity", .map(vm(("update_result", .map(vm(
