...
final class PipelineTest: XCTestCase {
  ...
  private func plClient(_ sdkopts: VMap?) -> (BluefinShieldconexMgmtSDK, Utility) {
    let client = BluefinShieldconexMgmtSDK.testSDK(nil, sdkopts)
    return (client, client.getUtility())
  }

  private func plCtx(_ client: BluefinShieldconexMgmtSDK, _ utility: Utility, _ ctrl: VMap?) -> Context {
  ...
  private func errCode(_ act: () throws -> Void) -> String {
    do { try act(); return "" } catch let e as BluefinShieldconexMgmtError { return e.code } catch { return "" }
  }
  ...
  private func utilWith(_ client: BluefinShieldconexMgmtSDK, _ fetcher: @escaping FetcherFunc) -> Utility {
  ...
  func testMakeRequestTransportErrorCarriedOnResponse() {
    ...
    XCTAssertEqual((resp.err as? BluefinShieldconexMgmtError)?.code, "boom")
  }
