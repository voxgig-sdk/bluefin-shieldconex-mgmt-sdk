// transaction entity test (generated from the API model).

import XCTest

@testable import BluefinShieldconexMgmtSdk

final class TransactionEntityTest: XCTestCase {
  func testInstance() {
    let sdk = BluefinShieldconexMgmtSDK.testSDK(nil, nil)
    let ent = sdk.Transaction()
    XCTAssertEqual(ent.getName(), "transaction")
  }

  func testStream() async throws {
    // Seed two records (under the test feature's entity key) and activate
    // the streaming feature.
    let fixtures = vm(("entity", .map(vm(("transaction", .map(vm(
