// BluefinShieldconexMgmt SDK test runner - shared infrastructure for the generated test
...
enum SdkRunner {
  ...
  // three dirs from this file (swift/Tests/BluefinShieldconexMgmtSDKTests -> project).
  static func testJsonPath(_ file: String = #filePath) -> String {
    let dir = URL(fileURLWithPath: file)
      .deletingLastPathComponent()  // BluefinShieldconexMgmtSDKTests
  ...
  static func errCode(_ e: Error?) -> String {
    (e as? BluefinShieldconexMgmtError)?.code ?? ""
  }
  ...
  static func makeCtxFromMap(_ ctxmapIn: VMap?, _ client: BluefinShieldconexMgmtSDK?, _ utility: Utility?)
    -> Context
  {
    ...
    if let resMap = ctxmap.entries["result"]?.asMap {
      ...
      if let errMap = resMap.entries["err"]?.asMap, let msg = errMap.entries["message"]?.asString {
        ctx.result!.err = BluefinShieldconexMgmtError("", msg, nil)
      }
    }
  ...
  static func fixCtx(_ ctx: Context, _ client: BluefinShieldconexMgmtSDK) {
  ...
  static func errFromMap(_ m: VMap?) -> Error? {
    ...
    return BluefinShieldconexMgmtError(code, msg, nil)
  }
}
