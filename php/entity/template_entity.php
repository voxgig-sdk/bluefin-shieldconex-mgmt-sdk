...
class TemplateEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new TemplateEntity($this->_client, $opts);
    }

    /**
     * @param Template|array $args Template data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = BluefinShieldconexMgmtHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Template|array The current Template data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Template fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = BluefinShieldconexMgmtHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Template fields).
     */
     ...
     *
     * @param TemplateLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed TemplateLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Template|array The loaded Template as an assoc-array at the
     *   SDK boundary; throws BluefinShieldconexMgmtError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = BluefinShieldconexMgmtHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param TemplateListMatch|array|null $reqmatch Match filter (any subset
     *   of Template fields) as an assoc-array; TemplateListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Template[]|array A list of Template items as assoc-arrays at
     *   the SDK boundary; throws BluefinShieldconexMgmtError on failure (item-5 convention).
     */
     ...
     *
     * @param TemplateCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed TemplateCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Template|array The created Template as an assoc-array at the
     *   SDK boundary; throws BluefinShieldconexMgmtError on failure (item-5 convention).
     */
     ...
     *
     * @param TemplateRemoveMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; TemplateRemoveMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Template|array The removed Template as an assoc-array at the
     *   SDK boundary; throws BluefinShieldconexMgmtError on failure (item-5 convention).
     */
