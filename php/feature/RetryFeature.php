...
// BluefinShieldconexMgmt SDK retry feature
...
class BluefinShieldconexMgmtRetryFeature extends BluefinShieldconexMgmtBaseFeature
{
    ...
    public function init(BluefinShieldconexMgmtContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BluefinShieldconexMgmtContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BluefinShieldconexMgmtContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BluefinShieldconexMgmtContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
