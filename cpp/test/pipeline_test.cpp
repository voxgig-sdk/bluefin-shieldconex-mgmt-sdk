// BluefinShieldconexMgmt SDK — direct unit tests for the operation-pipeline utilities
...
static std::shared_ptr<BluefinShieldconexMgmtSDK> plClient(const Value& sdkopts) {
  return BluefinShieldconexMgmtSDK::testSDK(Value::undef(), sdkopts);
}

static CtxPtr plCtx(std::shared_ptr<BluefinShieldconexMgmtSDK> client, UtilityPtr utility, const Value& ctrl) {
  CtxSpec cs;
  cs.setOpname("load");
...
static std::string names(std::shared_ptr<BluefinShieldconexMgmtSDK> client) {
  std::string out;
