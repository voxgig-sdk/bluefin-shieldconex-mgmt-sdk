// Generated direct-call tests for the client entity (unit mode;
...
static ClientDirectSetup client_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void client_direct_list() {
  auto setup = client_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-client", "unit");
...
static void client_direct_load() {
  auto setup = client_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-client", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;
...
int main() {
  T_RUN(client_direct_list);
  T_RUN(client_direct_load);
  return sdktest::summary("client_direct_test");
}
