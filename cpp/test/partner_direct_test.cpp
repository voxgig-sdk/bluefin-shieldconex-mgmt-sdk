// Generated direct-call tests for the partner entity (unit mode;
...
static PartnerDirectSetup partner_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void partner_direct_list() {
  auto setup = partner_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-partner", "unit");
...
static void partner_direct_load() {
  auto setup = partner_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-partner", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;
...
int main() {
  T_RUN(partner_direct_list);
  T_RUN(partner_direct_load);
  return sdktest::summary("partner_direct_test");
}
