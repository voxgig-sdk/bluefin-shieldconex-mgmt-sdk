// Generated direct-call tests for the template entity (unit mode;
...
static TemplateDirectSetup template__direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void template__direct_list() {
  auto setup = template__direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-template", "unit");
...
static void template__direct_load() {
  auto setup = template__direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-template", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;
...
int main() {
  T_RUN(template__direct_list);
  T_RUN(template__direct_load);
  return sdktest::summary("template_direct_test");
}
