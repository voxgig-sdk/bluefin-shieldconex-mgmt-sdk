// Generated direct-call tests for the transaction entity (unit mode;
...
static TransactionDirectSetup transaction_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void transaction_direct_list() {
  auto setup = transaction_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-transaction", "unit");
...
static void transaction_direct_load() {
  auto setup = transaction_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-transaction", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;
...
int main() {
  T_RUN(transaction_direct_list);
  T_RUN(transaction_direct_load);
  return sdktest::summary("transaction_direct_test");
}
