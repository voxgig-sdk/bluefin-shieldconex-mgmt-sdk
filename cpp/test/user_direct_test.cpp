// Generated direct-call tests for the user entity (unit mode;
...
static UserDirectSetup user_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void user_direct_load() {
  auto setup = user_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-user", "unit");
...
int main() {
  T_RUN(user_direct_load);
  return sdktest::summary("user_direct_test");
}
