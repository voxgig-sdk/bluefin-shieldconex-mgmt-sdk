...
use bluefin_shieldconex_mgmt_sdk::core::helpers::{get_str, getp, ja, jo, setp, to_map, vfn};
...
use bluefin_shieldconex_mgmt_sdk::{
    ...
    BluefinShieldconexMgmtSDK, SdkResult, Spec, Utility, Value,
};
...
fn base_client() -> (Rc<BluefinShieldconexMgmtSDK>, Rc<Utility>) {
...
fn make_test_ctx(client: &Rc<BluefinShieldconexMgmtSDK>, utility: &Rc<Utility>) -> Rc<Context> {
...
fn make_test_full_ctx(client: &Rc<BluefinShieldconexMgmtSDK>, utility: &Rc<Utility>) -> Rc<Context> {
...
fn primary_fetcher_live() {
    ...
    let c = calls.clone();
    let live_client = BluefinShieldconexMgmtSDK::new(jo(vec![(
        "system",
        jo(vec![(
            "fetch",
            vfn(move |args| {
...
fn primary_fetcher_blocked_test_mode() {
    ...
    let blocked_client = BluefinShieldconexMgmtSDK::new(jo(vec![(
        "system",
