// Generated direct-call tests for the update_result entity (mirrors the
...
fn update_result_direct_setup(mockres: Value) -> UpdateResultDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFINSHIELDCONEXMGMT_TEST_UPDATE_RESULT_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFINSHIELDCONEXMGMT_TEST_UPDATE_RESULT_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return UpdateResultDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn update_result_direct_list() {
    let setup = update_result_direct_setup(ja(vec![
        jo(vec![("id", Value::str("direct01"))]),
        jo(vec![("id", Value::str("direct02"))]),
    ]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-list-update_result", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
