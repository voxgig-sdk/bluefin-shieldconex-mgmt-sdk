...
#[test]
fn user_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.user(Value::Noval);
    assert_eq!(ent.get_name(), "user");
}

#[test]
fn user_entity_stream() {
    ...
    let plainent = plainsdk.user(Value::Noval);
...
#[test]
fn user_entity_basic() {
    let setup = user_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["load"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("user.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINSHIELDCONEXMGMT_TEST_USER_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();

    // Bootstrap entity data from existing test data (no create step in flow).
    let user_ref01_data_raw = vs::items(&to_map(&getpath(&["existing", "user"], &setup.data)));
    let user_ref01_data = to_map(&vs::get_elem(
        &vs::get_elem(&user_ref01_data_raw, &Value::Num(0.0), Value::Noval),
        &Value::Num(1.0),
        Value::Noval,
    ));
    // LOAD
    let user_ref01_ent = client.user(Value::Noval);
    let user_ref01_match_dt0 = jo(vec![("id", getp(&user_ref01_data, "id"))]);
    ...
    assert_eq!(
        getp(&user_ref01_data_dt0_load_result, "id"),
        getp(&user_ref01_data, "id"),
        "expected load result id to match"
    );

}

fn user_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("user");
    entity_data_file.push("UserTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("user01"), Value::str("user02"), Value::str("user03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINSHIELDCONEXMGMT_TEST_USER_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINSHIELDCONEXMGMT_TEST_USER_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINSHIELDCONEXMGMT_TEST_USER_ENTID")) {
