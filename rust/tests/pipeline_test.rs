...
use bluefin_shieldconex_mgmt_sdk::core::helpers::{getp, getpath, ja, jo};
...
use bluefin_shieldconex_mgmt_sdk::{
    ...
    BluefinShieldconexMgmtSDK, Response, SdkResult, Spec, Utility, Value,
};

// plClient builds a client + isolated utility for pipeline utility tests.
fn pl_client(sdkopts: Value) -> (Rc<BluefinShieldconexMgmtSDK>, Rc<Utility>) {
...
fn util_with(client: &Rc<BluefinShieldconexMgmtSDK>, fetcher: FetcherFn) -> Rc<Utility> {
...
fn pipeline_feature_add_ordering_before_after_replace() {
    ...
    let names = |client: &Rc<BluefinShieldconexMgmtSDK>| -> String {
        client
            .features
            .borrow()
            .iter()
...
use bluefin_shieldconex_mgmt_sdk::core::helpers::get_str;
