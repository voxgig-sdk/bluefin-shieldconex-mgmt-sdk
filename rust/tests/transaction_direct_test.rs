// Generated direct-call tests for the transaction entity (mirrors the
...
fn transaction_direct_setup(mockres: Value) -> TransactionDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFINSHIELDCONEXMGMT_TEST_TRANSACTION_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFINSHIELDCONEXMGMT_TEST_TRANSACTION_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return TransactionDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn transaction_direct_list() {
    let setup = transaction_direct_setup(ja(vec![
        jo(vec![("id", Value::str("direct01"))]),
        jo(vec![("id", Value::str("direct02"))]),
    ]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-list-transaction", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
...
#[test]
fn transaction_direct_load() {
    let setup = transaction_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-transaction", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    let params = Value::empty_map();
    if setup.live {
        setp(&params, "id", getp(&setup.idmap, "transaction01"));
    } else {
    ...
    let result = client
        .direct(jo(vec![
            ("path", Value::str("transactions/{id}")),
            ("method", Value::str("GET")),
            ("params", params.clone()),
        ]))
        .expect("direct failed");
