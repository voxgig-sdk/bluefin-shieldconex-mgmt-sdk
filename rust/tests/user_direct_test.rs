// Generated direct-call tests for the user entity (mirrors the
...
fn user_direct_setup(mockres: Value) -> UserDirectSetup {
    load_env_local();
...
#[test]
fn user_direct_load() {
    let setup = user_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-user", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    if setup.live {
        setp(&params, "id", getp(&setup.idmap, "user01"));
    } else {
