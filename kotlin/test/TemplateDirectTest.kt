...
class TemplateDirectTest {
  ...
  fun directListTemplate() {
    ...
    val reason = RunnerSupport.skipReason("direct", "direct-list-template", mode)
  ...
  @Test
  fun directLoadTemplate() {
    val setup = directSetup(jm("id", "direct01"))
    val mode = if (setup.live) "live" else "unit"
    val reason = RunnerSupport.skipReason("direct", "direct-load-template", mode)
    Assumptions.assumeTrue(
      reason == null,
      if (reason == null || "" == reason) "skipped via sdk-test-control.json" else reason,
    )
  ...
  companion object {
    ...
    fun directSetup(mockres: Any?): DirectSetup {
      ...
      envm["BLUEFINSHIELDCONEXMGMT_TEST_TEMPLATE_ENTID"] = linkedMapOf<String, Any?>()
      ...
      if (live) {
        ...
        var idmap: MutableMap<String, Any?> = linkedMapOf()
        val entidRaw = env["BLUEFINSHIELDCONEXMGMT_TEST_TEMPLATE_ENTID"]
