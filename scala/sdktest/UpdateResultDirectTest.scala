// Generated direct-call tests for the update_result entity (mirrors the java
...
object UpdateResultDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-list-update_result") {
      ...
      rep.eq("direct-list-update_result.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-list-update_result.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-list-update_result.islist", result.get("data").isInstanceOf[JList[?]], "expected data to be an array, got " + result.get("data"))
      val listData = result.get("data").asInstanceOf[JList[Object]]
      rep.eqI("direct-list-update_result.size", 2, listData.size())
      rep.eqI("direct-list-update_result.calls", 1, setup.calls.size())
    }
