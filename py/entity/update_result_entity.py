...
class UpdateResultEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return UpdateResultEntity(self._client, opts)
    ...
    def data_get(self) -> UpdateResult:
    ...
    def match_get(self) -> UpdateResult:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[UpdateResult]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.UpdateResult().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: UpdateResultCreateData, ctrl=None) -> UpdateResult:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def update(self, reqdata: UpdateResultUpdateData, ctrl=None) -> UpdateResult:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
