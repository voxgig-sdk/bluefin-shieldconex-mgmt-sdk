...
class TestTemplateDirect:

    def test_should_direct_list_template(self):
        setup = _template_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-template", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_template(self):
        setup = _template_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-template", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _template_direct_setup(mockres):
