...
class TestPartnerDirect:

    def test_should_direct_list_partner(self):
        setup = _partner_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-partner", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_partner(self):
        setup = _partner_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-partner", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _partner_direct_setup(mockres):
