...
class TestUpdateResultDirect:

    def test_should_direct_list_update_result(self):
        setup = _update_result_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-update_result", "live" if setup["live"] else "unit")
        if _skip:
...
def _update_result_direct_setup(mockres):
