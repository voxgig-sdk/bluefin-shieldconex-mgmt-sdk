...
let () =
  test "client.entity_instance" (fun () ->
      let client = Sdk_client.test () in
      let ent = Sdk_client.client client Noval in
      check_str "name" ent.e_name "client")

let () =
  test "client.seeded_ops" (fun () ->
      let record = jo [("id", Str "client01")] in
      let seed = jo [("client",
                      jo [("client01", record)])] in
      ...
      let loaded = ent.e_load (jo [("id", Str "client01")]) Noval in
      check "load is a map" (ismap loaded);
      check_vstr "load id" (getp loaded "id") "client01";
      ())

let () =
  test "client.stream" (fun () ->
      let mk_seed () =
        jo [("client",
      ...
      let ent2 = Sdk_client.client client2 Noval in
      let n = ref 0 in
      ...
      if has_streaming then begin
        ...
        let sent = Sdk_client.client sclient Noval in
        ...
        let cent = Sdk_client.client cclient Noval in
