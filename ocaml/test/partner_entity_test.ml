...
let () =
  test "partner.entity_instance" (fun () ->
      let client = Sdk_client.test () in
      let ent = Sdk_client.partner client Noval in
      check_str "name" ent.e_name "partner")

let () =
  test "partner.seeded_ops" (fun () ->
      let record = jo [("id", Str "partner01")] in
      let seed = jo [("partner",
                      jo [("partner01", record)])] in
      ...
      let loaded = ent.e_load (jo [("id", Str "partner01")]) Noval in
      check "load is a map" (ismap loaded);
      check_vstr "load id" (getp loaded "id") "partner01";
      ())

let () =
  test "partner.stream" (fun () ->
      let mk_seed () =
        jo [("partner",
      ...
      let ent2 = Sdk_client.partner client2 Noval in
      let n = ref 0 in
      ...
      if has_streaming then begin
        ...
        let sent = Sdk_client.partner sclient Noval in
        ...
        let cent = Sdk_client.partner cclient Noval in
