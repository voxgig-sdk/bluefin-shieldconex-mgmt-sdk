(* Generated template entity test. *)
...
let () =
  test "template.entity_instance" (fun () ->
      let client = Sdk_client.test () in
      let ent = Sdk_client.template client Noval in
      check_str "name" ent.e_name "template")

let () =
  test "template.seeded_ops" (fun () ->
      let record = jo [("id", Str "template01")] in
      let seed = jo [("template",
                      jo [("template01", record)])] in
      ...
      let loaded = ent.e_load (jo [("id", Str "template01")]) Noval in
      check "load is a map" (ismap loaded);
      check_vstr "load id" (getp loaded "id") "template01";
      ())

let () =
  test "template.stream" (fun () ->
      let mk_seed () =
        jo [("template",
      ...
      let ent2 = Sdk_client.template client2 Noval in
      let n = ref 0 in
      ...
      if has_streaming then begin
        ...
        let sent = Sdk_client.template sclient Noval in
        ...
        let cent = Sdk_client.template cclient Noval in
