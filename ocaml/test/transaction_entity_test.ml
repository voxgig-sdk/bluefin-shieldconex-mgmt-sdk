(* Generated transaction entity test. *)
...
let () =
  test "transaction.entity_instance" (fun () ->
      let client = Sdk_client.test () in
      let ent = Sdk_client.transaction client Noval in
      check_str "name" ent.e_name "transaction")

let () =
  test "transaction.seeded_ops" (fun () ->
      let record = jo [("id", Str "transaction01")] in
      let seed = jo [("transaction",
                      jo [("transaction01", record)])] in
      ...
      let loaded = ent.e_load (jo [("id", Str "transaction01")]) Noval in
      check "load is a map" (ismap loaded);
      check_vstr "load id" (getp loaded "id") "transaction01";
      ())

let () =
  test "transaction.stream" (fun () ->
      let mk_seed () =
        jo [("transaction",
      ...
      let ent2 = Sdk_client.transaction client2 Noval in
      let n = ref 0 in
      ...
      if has_streaming then begin
        ...
        let sent = Sdk_client.transaction sclient Noval in
        ...
        let cent = Sdk_client.transaction cclient Noval in
