(* Generated update_result entity test. *)
...
let () =
  test "update_result.entity_instance" (fun () ->
      let client = Sdk_client.test () in
      let ent = Sdk_client.update_result client Noval in
      check_str "name" ent.e_name "update_result")

let () =
  test "update_result.seeded_ops" (fun () ->
      let record = jo [("id", Str "update_result01")] in
      let seed = jo [("update_result",
                      jo [("update_result01", record)])] in
...
let () =
  test "update_result.stream" (fun () ->
      ...
      let ent2 = Sdk_client.update_result client2 Noval in
      let n = ref 0 in
      ...
      if has_streaming then begin
        ...
        let sent = Sdk_client.update_result sclient Noval in
        ...
        let cent = Sdk_client.update_result cclient Noval in
