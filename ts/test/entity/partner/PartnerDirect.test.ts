...
describe('PartnerDirect', async () => {
  ...
  test('direct-load-partner', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-partner', setup.live)) return
  ...
  test('direct-list-partner', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-partner', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['BLUEFINSHIELDCONEXMGMT_TEST_PARTNER_ENTID']
