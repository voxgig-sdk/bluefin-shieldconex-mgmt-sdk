...
describe('ClientDirect', async () => {
  ...
  test('direct-load-client', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-client', setup.live)) return
  ...
  test('direct-list-client', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-client', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['BLUEFINSHIELDCONEXMGMT_TEST_CLIENT_ENTID']
