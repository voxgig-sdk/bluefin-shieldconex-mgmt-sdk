...
describe('UpdateResultDirect', async () => {
  ...
  test('direct-list-update_result', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-update_result', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['BLUEFINSHIELDCONEXMGMT_TEST_UPDATE_RESULT_ENTID']
