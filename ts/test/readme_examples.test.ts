...
const SDK_NAME = 'BluefinShieldconexMgmtSDK'
...
const TEST_SEED = {"entity":{"client":{"test01":{"id":"test01"}},"clone":{"test01":{"id":"test01"}},"partner":{"test01":{"id":"test01"}},"template":{"test01":{"id":"test01"}},"transaction":{"test01":{"id":"test01"}},"update_result":{"test01":{"id":"test01"}},"user":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+BluefinShieldconexMgmtSDK\b/.test(code) ||
    /\bBluefinShieldconexMgmtSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BluefinShieldconexMgmtSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BluefinShieldconexMgmtSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
