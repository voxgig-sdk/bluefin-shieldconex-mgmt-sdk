// BluefinShieldconexMgmt SDK utility: makeError - the single error surface of the
// pipeline. Throws the wrapped BluefinShieldconexMgmtError unless the per-call ctrl
...
func makeErrorUtil(_ ctx: Context, _ err: Error?) throws -> Value {
  ...
  var msg = "BluefinShieldconexMgmtSDK: " + opname + ": " + errmsg
  ...
  let code = (e as? BluefinShieldconexMgmtError)?.code ?? ""
  let sdkErr = BluefinShieldconexMgmtError(code, msg, ctx)
