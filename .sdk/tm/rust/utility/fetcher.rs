...
fn default_http_fetch(fullurl: &str, fetchdef: &Value) -> Result<Value, BluefinShieldconexMgmtError> {
    ...
    if !has_ua {
        req = req.set("User-Agent", "Mozilla/5.0 (compatible; BluefinShieldconexMgmtSDK/1.0)");
    }
    ...
    let resp = match sent {
        ...
        Err(e) => {
            return Err(BluefinShieldconexMgmtError::new("fetch_transport", &format!("{}", e)));
        }
    };
    ...
    let body_txt = resp
        .into_string()
        .map_err(|e| BluefinShieldconexMgmtError::new("fetch_body", &format!("{}", e)))?;
