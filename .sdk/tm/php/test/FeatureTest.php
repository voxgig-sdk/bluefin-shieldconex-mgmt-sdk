...
// BluefinShieldconexMgmt SDK feature test
//
...
class FtCtrl extends BluefinShieldconexMgmtControl
{
...
class FtClient
{
    ...
    private BluefinShieldconexMgmtUtility $utility;

    public function __construct(BluefinShieldconexMgmtUtility $utility, array $options)
    {
    ...
    public function get_utility(): BluefinShieldconexMgmtUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BluefinShieldconexMgmtContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public BluefinShieldconexMgmtContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BluefinShieldconexMgmtContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BluefinShieldconexMgmtConfig::make_feature($name);
    ...
    private static function buildUrl(BluefinShieldconexMgmtSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BluefinShieldconexMgmtError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BluefinShieldconexMgmtResponse($fetched);
        }
    ...
    private function fail(BluefinShieldconexMgmtContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BluefinShieldconexMgmtContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
