# BluefinShieldconexMgmt SDK utility
#
...
defmodule BluefinShieldconexMgmt.Utility do
  alias Voxgig.Struct, as: S
  alias BluefinShieldconexMgmt.Helpers, as: H
  alias BluefinShieldconexMgmt.{Context, Spec, Result, Response, Operation}

  @default_user_agent "Mozilla/5.0 (compatible; BluefinShieldconexMgmtSDK/1.0)"
  ...
  def make_error_impl(ctx, err) do
    ...
    errmsg =
      cond do
        match?(%BluefinShieldconexMgmt.Error{}, err) -> err.msg
    ...
    msg = "BluefinShieldconexMgmtSDK: " <> opname <> ": " <> (errmsg || "")
    msg = clean(ctx, msg)
    ...
    sdk_err = %BluefinShieldconexMgmt.Error{
      code: "",
      msg: msg,
    ...
    sdk_err = if match?(%BluefinShieldconexMgmt.Error{}, err), do: %{sdk_err | code: err.code}, else: sdk_err
  ...
  defp err_msg(e) do
    cond do
      match?(%BluefinShieldconexMgmt.Error{}, e) -> e.msg
  ...
  defp safe_json(str) do
    try do
      {:ok, BluefinShieldconexMgmt.Json.parse(str)}
    rescue
      _ -> :error
    end
  end
