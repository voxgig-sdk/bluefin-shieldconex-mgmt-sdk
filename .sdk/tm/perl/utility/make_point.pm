# BluefinShieldconexMgmt SDK utility: make_point
...
$REGISTRY{make_point} = sub {
  ...
  if ($ctx->{out}{point}) {
    ...
    return (undef, $preset)
      if Scalar::Util::blessed($preset) && $preset->isa('BluefinShieldconexMgmtError');
    $ctx->{point} = $preset;
  ...
  my $allow_op = BluefinShieldconexMgmtHelpers::gpath($options, 'allow.op');
  ...
  if (1 == @{ $op->{points} }) {
    ...
    for my $p (@{ $op->{points} }) {
      $point = $p;
      my $select_def = BluefinShieldconexMgmtHelpers::to_map(BluefinShieldconexMgmtHelpers::gp($p, 'select'));
      my $found = 1;

      if ($selector && $select_def) {
        my $exist = BluefinShieldconexMgmtHelpers::gp($select_def, 'exist');
        if (Voxgig::Struct::islist($exist)) {
          for my $ek (@$exist) {
            my $rv = BluefinShieldconexMgmtHelpers::gp($reqselector, "$ek");
            my $sv = BluefinShieldconexMgmtHelpers::gp($selector, "$ek");
      ...
      if ($found) {
        my $req_action = BluefinShieldconexMgmtHelpers::gp($reqselector, '$action');
        my $select_action = BluefinShieldconexMgmtHelpers::gp($select_def, '$action');
        $found = 0 unless BluefinShieldconexMgmtHelpers::eqv($req_action, $select_action);
      }
    ...
    if ($reqselector) {
      ...
      if (defined $req_action && $point) {
        my $point_select = BluefinShieldconexMgmtHelpers::to_map(BluefinShieldconexMgmtHelpers::gp($point, 'select'));
        my $point_action = BluefinShieldconexMgmtHelpers::gp($point_select, '$action');
        unless (BluefinShieldconexMgmtHelpers::eqv($req_action, $point_action)) {
