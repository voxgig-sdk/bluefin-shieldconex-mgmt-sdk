...
sub resp {
  ...
  return BluefinShieldconexMgmtResponse->new({
    'status' => $status,
...
sub spec_of {
  my (%map) = @_;
  return BluefinShieldconexMgmtSpec->new({
...
sub is_sdk_error {
  my ($err) = @_;
  return Scalar::Util::blessed($err) && $err->isa('BluefinShieldconexMgmtError');
}
...

{
  ...
  $ctx->{result} = BluefinShieldconexMgmtResult->new({});
...

{
  ...
  $ctx->{spec} = spec_of();
  $ctx->{result} = BluefinShieldconexMgmtResult->new({ 'ok' => 1, 'resdata' => [] });
  my ($result, $err) = $utility->{make_result}->($ctx);
...
{
  my $ctx = make_ctx();
  my $preset = BluefinShieldconexMgmtResult->new({ 'ok' => 1 });
...
{
  my $boom = BluefinShieldconexMgmtError->new('boom', 'boom');
...

{
  ...
  $u->{make_fetch_def} = sub {
    return (undef, BluefinShieldconexMgmtError->new('fetchdef_boom', 'boom'));
  };
  my $ctx = make_ctx(utility => $u);
  $ctx->{spec} = request_spec();
  my ($response, $err) = $u->{make_request}->($ctx);
...
{
  my $ctx = make_ctx();
  $ctx->{result} = BluefinShieldconexMgmtResult->new({ 'ok' => 1, 'resdata' => 42 });
  is($utility->{done}->($ctx), 42, 'done returns resdata on success');
}

{
  my $ctx = make_ctx();
  $ctx->{result} = BluefinShieldconexMgmtResult->new({ 'ok' => 0 });
...

{
  ...
  $ctx->{result} = BluefinShieldconexMgmtResult->new({ 'ok' => 1, 'resdata' => 7 });
...

{
  ...
  $ctx->{result} = BluefinShieldconexMgmtResult->new({ 'ok' => 0, 'resdata' => 'fallback' });
...
{
  my $oclient = BluefinShieldconexMgmtSDK->test(undef, undef);
  ...
  $oclient->{features} = [];
  my $a = BluefinShieldconexMgmtBaseFeature->new;
  my $b = BluefinShieldconexMgmtBaseFeature->new;
...
sub named_feature {
  my ($name) = @_;
  my $f = BluefinShieldconexMgmtBaseFeature->new;
  $f->{name} = $name;
  return $f;
}
...

{
  ...
  ok(BluefinShieldconexMgmtHelpers::is_true($o2->{feature}{metrics}{active}),
    'feature order: array normalized to a map (metrics opts preserved)');
  ok(BluefinShieldconexMgmtHelpers::is_true($o2->{feature}{test}{active}),
...
sub auth_ctx {
  ...
  my $ctx = BluefinShieldconexMgmtContext->new({
    'client' => PipelineOptClient->new($options),
    'utility' => $utility,
    'opname' => 'load',
  }, undef);
  $ctx->{spec} = defined $headers
    ? BluefinShieldconexMgmtSpec->new({ 'headers' => $headers, 'step' => 's' })
    : undef;
  return $ctx;
}
...
{
  my $ctx = make_ctx();
  $ctx->{response} = BluefinShieldconexMgmtResponse->new({ 'status' => 200 });
  $ctx->{result} = BluefinShieldconexMgmtResult->new({});
...
{
  my $ctx = make_ctx();
  $ctx->{response} = BluefinShieldconexMgmtResponse->new({
