...
{
  my $sdk = BluefinShieldconexMgmtSDK->test({ 'net' => { 'offline' => 1 } }, undef);
...
{
  my $sdk = BluefinShieldconexMgmtSDK->test({ 'net' => { 'failTimes' => 1, 'failStatus' => 503 } }, undef);
  my $res = $sdk->direct({ 'path' => '/ping' });
...
{
  my $sdk = BluefinShieldconexMgmtSDK->test({ 'net' => { 'errorTimes' => 1 } }, undef);
  my $res = $sdk->direct({ 'path' => '/ping' });
...
{
  my $delay = 60;
  my $sdk = BluefinShieldconexMgmtSDK->test({ 'net' => { 'latency' => $delay } }, undef);
...
{
  my $sdk = BluefinShieldconexMgmtSDK->test(undef, undef);
