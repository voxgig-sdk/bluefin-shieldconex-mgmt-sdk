// Command BluefinShieldconexMgmt-cli is a CLI + REPL for the BluefinShieldconexMgmt SDK,
...
const prompt = "bluefin-shieldconex-mgmt"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "client clone partner template transaction update_result user"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BLUEFIN_SHIELDCONEX_MGMT_APIKEY (api key), BLUEFIN_SHIELDCONEX_MGMT_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BLUEFIN_SHIELDCONEX_MGMT_APIKEY carries the API key and
	// BLUEFIN_SHIELDCONEX_MGMT_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BLUEFIN_SHIELDCONEX_MGMT_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BLUEFIN_SHIELDCONEX_MGMT_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBluefinShieldconexMgmtSDK(opts)
