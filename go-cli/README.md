# bluefin-shieldconex-mgmt-cli

AQL-driven command-line client **and** interactive REPL for the BluefinShieldconexMgmt
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bluefin-shieldconex-mgmt-cli)
make build

# 2. See usage (words, entities, env vars)
./bluefin-shieldconex-mgmt-cli --help
...
./bluefin-shieldconex-mgmt-cli list client
./bluefin-shieldconex-mgmt-cli load 1 client            # {id:1} shorthand
./bluefin-shieldconex-mgmt-cli load '{id:1}' client       # explicit match map
./bluefin-shieldconex-mgmt-cli list clone

# 5. Override the API base URL for a single call
BLUEFIN_SHIELDCONEX_MGMT_BASE=https://api.example.com ./bluefin-shieldconex-mgmt-cli list client

# 6. No arguments -> interactive REPL
./bluefin-shieldconex-mgmt-cli
bluefin-shieldconex-mgmt> list client
bluefin-shieldconex-mgmt> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bluefin-shieldconex-mgmt-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/bluefin-shieldconex-mgmt-cli list client
   ```
...
```sh
./bluefin-shieldconex-mgmt-cli load 1 client          # scalar shorthand for {id:1}
./bluefin-shieldconex-mgmt-cli load '{id:1}' client     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `bluefin-shieldconex-mgmt>`). Each line is
evaluated as its own AQL expression:

```text
$ ./bluefin-shieldconex-mgmt-cli
bluefin-shieldconex-mgmt> list client
bluefin-shieldconex-mgmt> /help
bluefin-shieldconex-mgmt> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `client`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/bluefin-shieldconex-mgmt-cli`. |
...
(`./bluefin-shieldconex-mgmt-cli <expr>`) and interactively (the REPL), and expressions compose the
