...
public class PipelineTest
{
    ...
    private static (BluefinShieldconexMgmtSDK, Utility) PlClient(Dictionary<string, object?>? sdkopts)
    {
        var client = BluefinShieldconexMgmtSDK.TestSDK(null, sdkopts);
        return (client, client.GetUtility());
    }

    private static Context PlCtx(BluefinShieldconexMgmtSDK client, Utility utility,
    ...
    private static string ErrCode(Action act)
    {
        ...
        catch (BluefinShieldconexMgmtError err)
        {
            return err.Code;
        }
        catch (Exception)
    ...
    private static Utility UtilWith(BluefinShieldconexMgmtSDK client, FetcherFunc fetcher)
    {
    ...
    public void MakeRequestTransportErrorCarriedOnResponse()
    {
        ...
        Assert.True(resp.Err is BluefinShieldconexMgmtError pe && pe.Code == "boom",
