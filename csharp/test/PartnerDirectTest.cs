// partner direct API tests (generated from the API model).
...
public class PartnerDirectTest
{
    [Fact]
    public void DirectList()
    {
        var setup = PartnerDirectSetup(new List<object?>
        {
        ...
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-list-partner", _mode);
        if (_shouldSkip)
        {
    ...
    {
        var setup = PartnerDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-partner", _mode);
        if (_shouldSkip)
        {
            return; // skipped via sdk-test-control.json
        }
    ...
    private class PartnerDirectSetupResult
    {
    ...
    private static PartnerDirectSetupResult PartnerDirectSetup(object? mockres)
    {
        ...
        {
            ["BLUEFINSHIELDCONEXMGMT_TEST_PARTNER_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["BLUEFINSHIELDCONEXMGMT_TEST_PARTNER_ENTID"];
            ...
            return new PartnerDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
