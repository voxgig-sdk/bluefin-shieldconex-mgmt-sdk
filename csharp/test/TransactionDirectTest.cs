// transaction direct API tests (generated from the API model).
...
public class TransactionDirectTest
{
    ...
    {
        var setup = TransactionDirectSetup(new List<object?>
        {
        ...
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-list-transaction", _mode);
        if (_shouldSkip)
        {
    ...
    {
        var setup = TransactionDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-transaction", _mode);
        if (_shouldSkip)
        {
            return; // skipped via sdk-test-control.json
        }
    ...
    private class TransactionDirectSetupResult
    {
    ...
    private static TransactionDirectSetupResult TransactionDirectSetup(object? mockres)
    {
        ...
        {
            ["BLUEFINSHIELDCONEXMGMT_TEST_TRANSACTION_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["BLUEFINSHIELDCONEXMGMT_TEST_TRANSACTION_ENTID"];
            ...
            return new TransactionDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
