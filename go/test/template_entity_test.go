...
func TestTemplateEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFINSHIELDCONEXMGMT_TEST_TEMPLATE_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		templateRef01DataDt0LoadResult := core.ToMapAny(templateRef01DataDt0Loaded)
...
func templateBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFINSHIELDCONEXMGMT_TEST_TEMPLATE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFINSHIELDCONEXMGMT_TEST_TEMPLATE_ENTID"])
