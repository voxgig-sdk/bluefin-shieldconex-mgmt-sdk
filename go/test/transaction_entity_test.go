...
func TestTransactionEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFINSHIELDCONEXMGMT_TEST_TRANSACTION_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		transactionRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.transaction", setup.data)))
		var transactionRef01Data map[string]any
		if len(transactionRef01DataRaw) > 0 {
			transactionRef01Data = core.ToMapAny(transactionRef01DataRaw[0][1])
		}
		// Discard guards against Go's unused-var check when the flow's steps
		// happen not to consume the bootstrap data (e.g. list-only flows).
		_ = transactionRef01Data
		...
		_, transactionRef01ListOk := transactionRef01ListResult.([]any)
...
func transactionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFINSHIELDCONEXMGMT_TEST_TRANSACTION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFINSHIELDCONEXMGMT_TEST_TRANSACTION_ENTID"])
