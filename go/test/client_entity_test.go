...
func TestClientEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFINSHIELDCONEXMGMT_TEST_CLIENT_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		clientRef01Data = core.ToMapAny(clientRef01DataResult)
		...
		clientRef01DataDt0LoadResult := core.ToMapAny(clientRef01DataDt0Loaded)
...
func clientBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFINSHIELDCONEXMGMT_TEST_CLIENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFINSHIELDCONEXMGMT_TEST_CLIENT_ENTID"])
