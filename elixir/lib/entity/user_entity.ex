...
defmodule BluefinShieldconexMgmt.Entity.User do
  ...
  def new(client, entopts \\ nil) do
    EntityBase.construct(__MODULE__, client, "user", entopts)
  end
  ...
  # Returns the user entity map (BluefinShieldconexMgmt.Types.user/0) on
  # success; pipeline errors surface as the error value built by
  # Utility.make_error (shape is utility-configurable), hence term().
  @spec load(map(), BluefinShieldconexMgmt.Types.user_load_match() | nil, map() | nil) :: term()
