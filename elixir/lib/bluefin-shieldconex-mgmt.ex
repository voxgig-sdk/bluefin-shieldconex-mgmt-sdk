...
defmodule BluefinShieldconexMgmt do
  alias Voxgig.Struct, as: S
  alias BluefinShieldconexMgmt.Helpers, as: H
  alias BluefinShieldconexMgmt.{Utility, Context, Spec}
  ...
  def new(options \\ nil) do
    ...
    config = BluefinShieldconexMgmt.Config.make_config()
    ...
    if S.islist(feature_order) and S.size(feature_order) > 0 do
      Enum.each(0..(S.size(feature_order) - 1), fn i ->
        ...
        if fom != nil and S.getprop(fom, "active") == true do
          Utility.feature_add(rootctx, BluefinShieldconexMgmt.Features.make_feature(fname))
        end
      end)
    end
      ...
      _ ->
        ...
        cond do
          ...
          S.ismap(fetched) ->
            ...
            content_length =
              if S.ismap(headers), do: BluefinShieldconexMgmt.Feature.header_get(headers, "content-length"), else: nil
  ...
  @doc "Entity factory for client."
  def client(client, entopts \\ nil) do
    BluefinShieldconexMgmt.Entity.Client.new(client, entopts)
  end

  @doc "Entity factory for clone."
  def clone(client, entopts \\ nil) do
    BluefinShieldconexMgmt.Entity.Clone.new(client, entopts)
  end

  @doc "Entity factory for partner."
  def partner(client, entopts \\ nil) do
    BluefinShieldconexMgmt.Entity.Partner.new(client, entopts)
  end

  @doc "Entity factory for template."
  def template(client, entopts \\ nil) do
    BluefinShieldconexMgmt.Entity.Template.new(client, entopts)
  end

  @doc "Entity factory for transaction."
  def transaction(client, entopts \\ nil) do
    BluefinShieldconexMgmt.Entity.Transaction.new(client, entopts)
  end

  @doc "Entity factory for update_result."
  def update_result(client, entopts \\ nil) do
    BluefinShieldconexMgmt.Entity.UpdateResult.new(client, entopts)
  end

  @doc "Entity factory for user."
  def user(client, entopts \\ nil) do
    BluefinShieldconexMgmt.Entity.User.new(client, entopts)
  end
