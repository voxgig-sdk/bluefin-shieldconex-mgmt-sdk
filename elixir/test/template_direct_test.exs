# Template direct test (offline, mock system.fetch)

defmodule BluefinShieldconexMgmt.TemplateDirectTest do
  use ExUnit.Case
  ...
  test "should direct list template" do
    canned = S.jt([S.jm(["id", "direct01"]), S.jm(["id", "direct02"])])
    sdk = mk(canned)
    res = BluefinShieldconexMgmt.direct(sdk, H.deep(%{"path" => "/template", "method" => "GET", "params" => %{}}))
  ...
  test "should direct load template" do
    canned = S.jm(["id", "direct01"])
    sdk = mk(canned)
    res = BluefinShieldconexMgmt.direct(sdk, H.deep(%{"path" => "/template/direct01", "method" => "GET", "params" => %{}}))
    assert S.getprop(res, "ok") == true
    assert H.to_int(S.getprop(res, "status")) == 200
