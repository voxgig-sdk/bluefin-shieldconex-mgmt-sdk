...
defmodule BluefinShieldconexMgmt.PrimaryUtilityTest do
  use ExUnit.Case

  alias Voxgig.Struct, as: S
  alias BluefinShieldconexMgmt.Helpers, as: H
  alias BluefinShieldconexMgmt.{Context, Utility, Spec, Result}

  defp client, do: BluefinShieldconexMgmt.test()

  defp ctx(client, opname \\ "load") do
    Context.new(S.jm(["opname", opname]), BluefinShieldconexMgmt.get_root_ctx(client))
  end
  ...
  test "prepare_auth sets an authorization header when an apikey is present" do
    c = BluefinShieldconexMgmt.new(H.deep(%{"apikey" => "secret"}))
    ctx = ctx(c)
  ...
  test "make_options derives a clean key regex" do
    c = BluefinShieldconexMgmt.test()
    opts = BluefinShieldconexMgmt.options_map(c)
