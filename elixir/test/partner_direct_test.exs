# Partner direct test (offline, mock system.fetch)

defmodule BluefinShieldconexMgmt.PartnerDirectTest do
  use ExUnit.Case
  ...
  test "should direct list partner" do
    canned = S.jt([S.jm(["id", "direct01"]), S.jm(["id", "direct02"])])
    sdk = mk(canned)
    res = BluefinShieldconexMgmt.direct(sdk, H.deep(%{"path" => "/partner", "method" => "GET", "params" => %{}}))
  ...
  test "should direct load partner" do
    canned = S.jm(["id", "direct01"])
    sdk = mk(canned)
    res = BluefinShieldconexMgmt.direct(sdk, H.deep(%{"path" => "/partner/direct01", "method" => "GET", "params" => %{}}))
    assert S.getprop(res, "ok") == true
    assert H.to_int(S.getprop(res, "status")) == 200
