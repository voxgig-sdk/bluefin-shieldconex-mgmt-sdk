...
class TransactionEntity
  ...
  def make
    opts = @_entopts.dup
    TransactionEntity.new(@_client, opts)
  end
  ...
  # @return [Transaction, Hash] the current Transaction data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Transaction fields)
  def match_get
  ...
  #
  # @param reqmatch [TransactionLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Transaction.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Transaction, Hash] the loaded Transaction; raises BluefinShieldconexMgmtError on failure
  ...
  #
  # @param reqmatch [TransactionListMatch, Hash, nil] match filter (any subset of
  #   Transaction fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Transaction>, Array] the matching Transaction items; raises BluefinShieldconexMgmtError on failure
