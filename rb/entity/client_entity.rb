...
class ClientEntity
  ...
  def make
    opts = @_entopts.dup
    ClientEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BluefinShieldconexMgmtHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Client, Hash] the current Client data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BluefinShieldconexMgmtHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Client fields)
  def match_get
  ...
  #
  # @param reqmatch [ClientLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Client.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Client, Hash] the loaded Client; raises BluefinShieldconexMgmtError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = BluefinShieldconexMgmtHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [ClientListMatch, Hash, nil] match filter (any subset of
  #   Client fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Client>, Array] the matching Client items; raises BluefinShieldconexMgmtError on failure
  ...
  # Create a new Client.
  #
  # @param reqdata [ClientCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Client, Hash] the created Client; raises BluefinShieldconexMgmtError on failure
  ...
  #
  # @param reqmatch [ClientRemoveMatch, Hash, nil] match criteria (id/query fields)
  # @param ctrl [Object, nil] optional per-call control
  # @return [Client, Hash] the removed Client; raises BluefinShieldconexMgmtError on failure
