...
module BluefinShieldconexMgmtUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BluefinShieldconexMgmtResponse.new({})
    ...
    if fetch_err
      ...
      response = BluefinShieldconexMgmtResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BluefinShieldconexMgmtResponse.new(fetched)
    else
