...
class UpdateResultDirectTest < Minitest::Test
  def test_direct_list_update_result
    setup = update_result_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-update_result", setup[:live] ? "live" : "unit")
    if _should_skip
...
def update_result_direct_setup(mockres)
