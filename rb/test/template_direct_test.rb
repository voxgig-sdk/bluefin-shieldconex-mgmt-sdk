...
class TemplateDirectTest < Minitest::Test
  def test_direct_list_template
    ...
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-template", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_template
    setup = template_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-template", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def template_direct_setup(mockres)
