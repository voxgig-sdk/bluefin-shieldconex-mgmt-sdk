  ...
  describe('UserEntity', () {
    ...
    test('basic', (t) async {
      ...
      for (final op in ['load']) {
        if (maybeSkipControl(t, 'entityOp', 'user.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_SHIELDCONEX_MGMT_TEST_USER_ENTID JSON to run live');
        return;
      }
      ...
      final user_ref01_data =
          (setup['data']['existing']['user'] as Map).values.first;
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_SHIELDCONEX_MGMT_TEST_USER_ENTID'];
