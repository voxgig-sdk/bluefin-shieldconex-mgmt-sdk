...
void tests() {
  describe('TransactionDirect', () {
    ...
    test('direct-load-transaction', (t) async {
      final setup = directSetup({'id': 'direct01'});
      if (maybeSkipControl(
          t, 'direct', 'direct-load-transaction', true == setup['live'])) {
        return;
      }
    ...
    test('direct-list-transaction', (t) async {
      final setup = directSetup([
        {'id': 'direct01'},
        {'id': 'direct02'}
      ]);
      if (maybeSkipControl(
          t, 'direct', 'direct-list-transaction', true == setup['live'])) {
        return;
      }
      final client = setup['client'];
      final calls = setup['calls'];
...
Map<String, dynamic> directSetup([dynamic mockres]) {
  ...
  final env = envOverride({
    'BLUEFINSHIELDCONEXMGMT_TEST_TRANSACTION_ENTID': <String, dynamic>{},
  ...
  if (live) {
    ...
    dynamic idmap = env['BLUEFINSHIELDCONEXMGMT_TEST_TRANSACTION_ENTID'];
