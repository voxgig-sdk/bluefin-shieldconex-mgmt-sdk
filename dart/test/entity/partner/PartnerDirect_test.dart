...
void tests() {
  describe('PartnerDirect', () {
    ...
    test('direct-load-partner', (t) async {
      final setup = directSetup({'id': 'direct01'});
      if (maybeSkipControl(
          t, 'direct', 'direct-load-partner', true == setup['live'])) {
        return;
      }
    ...
    test('direct-list-partner', (t) async {
      final setup = directSetup([
        {'id': 'direct01'},
        {'id': 'direct02'}
      ]);
      if (maybeSkipControl(
          t, 'direct', 'direct-list-partner', true == setup['live'])) {
        return;
      }
      final client = setup['client'];
      final calls = setup['calls'];
...
Map<String, dynamic> directSetup([dynamic mockres]) {
  ...
  final env = envOverride({
    'BLUEFINSHIELDCONEXMGMT_TEST_PARTNER_ENTID': <String, dynamic>{},
  ...
  if (live) {
    ...
    dynamic idmap = env['BLUEFINSHIELDCONEXMGMT_TEST_PARTNER_ENTID'];
