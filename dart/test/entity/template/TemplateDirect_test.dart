...
void tests() {
  describe('TemplateDirect', () {
    ...
    test('direct-load-template', (t) async {
      final setup = directSetup({'id': 'direct01'});
      if (maybeSkipControl(
          t, 'direct', 'direct-load-template', true == setup['live'])) {
        return;
      }
    ...
    test('direct-list-template', (t) async {
      final setup = directSetup([
        {'id': 'direct01'},
        {'id': 'direct02'}
      ]);
      if (maybeSkipControl(
          t, 'direct', 'direct-list-template', true == setup['live'])) {
        return;
      }
      final client = setup['client'];
      final calls = setup['calls'];
...
Map<String, dynamic> directSetup([dynamic mockres]) {
  ...
  final env = envOverride({
    'BLUEFINSHIELDCONEXMGMT_TEST_TEMPLATE_ENTID': <String, dynamic>{},
  ...
  if (live) {
    ...
    dynamic idmap = env['BLUEFINSHIELDCONEXMGMT_TEST_TEMPLATE_ENTID'];
