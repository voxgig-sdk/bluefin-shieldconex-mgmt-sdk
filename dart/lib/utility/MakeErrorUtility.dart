import '../BluefinShieldconexMgmtError.dart';
import '../Result.dart';
...
dynamic makeError(dynamic ctx, [dynamic err]) {
  ...
  final msg = 'BluefinShieldconexMgmtSDK: ' + opname.toString() + ': ' + errmsg(err);

  BluefinShieldconexMgmtError sdkerr;
  if (err is BluefinShieldconexMgmtError) {
    sdkerr = err;
    sdkerr.message = clean(ctx, msg);
  } else {
    final code = errcode(err);
    sdkerr = BluefinShieldconexMgmtError('' == code ? 'unknown' : code, clean(ctx, msg), ctx);
  }
